...
def plot_single_graph(client, table, interval):

    print(f"Plotting {client} {table} {interval} graph.")

    ##### database value collection #####

    try:  # database operations fail protection

        conn = database.create_connection(client)

        # skip graph if no data is found in the table for the interval
        if not database.db_records_in_timespan_exist(conn, table, interval):
            print(f"No {table} data yet for period {interval}. Skipping.\n")
...
def plot_multi_graph(client, table, interval):
    ...
    for deviceNbr in range(2, 12, 2):  # every other col in DB (5 double values)
        ...
        try:  # database operations fail protection
            ...
            # skip device if no data exists
            if data[-1][deviceNbr] is None:  # if last row doesn't contain valid data
                database.close_connection(conn)
                continue
